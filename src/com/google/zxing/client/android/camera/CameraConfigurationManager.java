...
final class CameraConfigurationManager {
	...
	void initFromCameraParameters(Camera camera) {
		...
		/**
		 * Camera.Parameters parameters = camera.getParameters(); WindowManager
		 ...
		 * height = display.getHeight(); // We're landscape-only, and have
		 * apparently seen issues with display // thinking it's portrait // when
		 * waking from sleep. If it's not landscape, assume it's mistaken // and
		 * reverse them: if (width < height) { Log.i(TAG,
		 * "Display reports portrait orientation; assuming this is incorrect");
		 * int temp = width; width = height; height = temp; } screenResolution =
		 * new Point(width, height); Log.i(TAG, "Screen resolution: " +
		 * screenResolution); cameraResolution =
		 * findBestPreviewSizeValue(parameters, screenResolution); Log.i(TAG,
